# %% [markdown]
# # Qwen3 FP8 TD-MoE Experiments
#
# Full MoE-only experiment entrypoint for Qwen3-30B-A3B-FP8.
# It benchmarks baseline and dense-reconstructed TD-MoE edited experts,
# then runs inference through the native FP8 expert path.
...
QWEN3_FP8_MODEL_NAME = os.environ.get("QWEN3_FP8_MODEL_NAME", "Qwen/Qwen3-30B-A3B-FP8")

OUTPUT_DIR = REPO_ROOT / "results_qwen3_30b_td_moe_fp8"
OUTPUT_JSON = OUTPUT_DIR / "qwen3_30b_td_moe_fp8_results.json"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# The loader has Qwen3 FP8-specific handling: non-quantized layers are BF16,
# expert tensors stay FP8 and are dequantized/requantized only around TD-MoE.
MODEL_DTYPE = "auto"
MODEL_STORAGE_BITS = 8
SCALE_STORAGE_BITS = 16
LORA_STORAGE_BITS = 16
DEVICE_MAP = os.environ.get("QWEN3_FP8_DEVICE_MAP", "auto")
...
cfg = RunnerConfig(
    ...
    model_name=QWEN3_FP8_MODEL_NAME,
...
display(pd.DataFrame([{
    ...
    "model_storage_bits": c.model_storage_bits,
...
# %%
n_layers = get_num_layers_from_config(QWEN3_FP8_MODEL_NAME)
TARGET_SETS = middle_out_expanding_layer_sets(
    n_layers,
    label_prefix="qwen3_30b_td_moe_fp8_middle_out",
...
TARGET_SETS_BY_MODEL = {QWEN3_FP8_MODEL_NAME: TARGET_SETS}
...
RECIPES = [
    RecipeSpec(
        label=f"td_moe_gate_up_fp8_cr{ratio_label(ratio)}",
        plans=[
            ModulePlan(
                "moe_experts",
                "td_moe",
